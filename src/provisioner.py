...
def main() -> None:
    ...
    try:
        ...
        arg = sys.argv[1:]
        if len(arg) == 0:
            msg_format.error_msg("No arguments provided.", endpoint)
            msg_format.general_msg("Valid arguments: 'swift', 'heat', 'guacamole', or 'full'", endpoint)
            return

        if arg[0] not in ["swift", "heat", "guacamole", "full"]:
            msg_format.error_msg(f"'{arg[0]}' is an invalid arguement.", endpoint)
            msg_format.general_msg("Valid arguments: 'swift', 'heat', 'guacamole', or 'full'", endpoint)
            return
        ...
        openstack_connect = connections.openstack_connection(heat_globals['cloud'], openstack_clouds, debug)

        if arg[0] == "swift":
            swift.provision(openstack_connect, globals, swift_globals, debug)
        elif arg[0] == "heat":
            if env_params:
                manage_ids.update_env(openstack_connect, global_dict, True, debug)
                heat_params, sec_params, env_params = manage_ids.update_ids(openstack_connect, [heat_params, sec_params, env_params], [], False, debug)
            heat.provision(openstack_connect, globals, heat_globals, heat_params, sec_params, debug)
        elif arg[0] == "guacamole":
            guacamole_connect = connections.guacamole_connection(guacamole_clouds, debug)
            guac.provision(openstack_connect, guacamole_connect, globals, guacamole_globals, heat_params, user_params, debug)
        elif arg[0] == "full":
            guacamole_connect = connections.guacamole_connection(guacamole_clouds, debug)
            swift.provision(openstack_connect, globals, swift_globals, debug)
            if env_params:
        ...
        msg_format.general_msg(f"Total time: {end - start:.2f} seconds", endpoint)

    except Exception as error:
        msg_format.error_msg(f"Error: {error}.\nTraceback: {traceback.format_exc()}", endpoint)
        sys.exit(1)

    msg_format.success_msg("Provisioning complete.", endpoint)
    sys.exit(0)
